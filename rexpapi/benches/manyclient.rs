...
use datacapsule::client::{run_client, p2pclient::ClientConnection};
use openssl::{
    ec::{EcGroup, EcKey},
    nid::Nid,
    pkey::{Private, Public},
...
async fn client_do_writes(cc: &mut ClientConnection, spubk: &EcKey<Public>) -> Vec<Hash> {
    let mut all_hashes_for_read = Vec::new();
    {
        ...
        // println!("write time after receive {:?}, got {:?}", tt.elapsed(), resp.len());
    }
    all_hashes_for_read
}
...
async fn main() {
    ...
    if UNIQUE_DCS {
        ...
        for n in 0..NUM_CLIENTS {
            let (c, s, dc) = original_client.new_data_capsule_request(&cpubk, &format!("benchmarkdc{n}"));
            reqs.push(c);
            syncs.push(s);
            dc_names.push(dc);
        }
        ...
        let (capsule_req, capsule_s, dc_name) = original_client.new_data_capsule_request(&cpubk, "benchmark");
    ...
    println!("TESTING REXP WITH {} CLIENTS {} UNIQUE {} RECORDS {} PER COMMIT", NUM_CLIENTS, UNIQUE_DCS, TOTAL_RECORDS, RECORDS_PER_COMMIT);


    let tt = Instant::now();
    let mut ffs = Vec::with_capacity(NUM_CLIENTS);
    for cc in &mut clients {
        ffs.push(client_do_writes(cc, &spubk));
    }
