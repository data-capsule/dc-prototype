...
use datacapsule::client::{run_client, p2pclient::{ClientConnection, ClientSync, Request}};
use openssl::{
    ec::{EcGroup, EcKey},
    nid::Nid,
    pkey::{Private, Public},
...
fn process_trace_raw(cc: &mut ClientConnection, kv: &mut HashMap<u64, Hash>, trace: &[YCSBOp]) -> (Vec<Request>, Vec<ClientSync>) {
    ...
    for op in trace {
        let (r, s) = match op {
            YCSBOp::Set(k, v) => {
                let (r, s, h) = cc.write_request(&v.as_bytes());
                kv.insert(*k, h);
                (r, s)
            },
            YCSBOp::Get(k) => {
        ...
        let (r, s) = match op {
            ...
            YCSBOp::Get(k) => {
                ...
                if unsigned_set.contains(k) {
                    let (r, s, _) = cc.commit_request(&unsigned, [0; 32]);
                    reqs.push(r);
                    syncs.push(s);
                    unsigned_set.clear();
                    unsigned.clear();
...
async fn main() {
    ...
    println!("REXP YCSB WORKLOAD {WORKLOAD} WOOHOO");
